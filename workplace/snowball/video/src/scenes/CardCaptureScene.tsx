import React from 'react';
import {
  AbsoluteFill,
  useCurrentFrame,
  useVideoConfig,
  spring,
} from 'remotion';
import { theme } from '../theme';
import { PhoneMockup } from '../components/PhoneMockup';

const BULLET_POINTS = [
  'Snap a photo, add notes, tap submit',
  'Vision AI extracts name, company, title, phone, email',
  'Auto-creates lead in Monday CRM with image attached',
  '800-card backfill as one-time onboarding',
];

const BULLET_DELAYS = [15, 22, 29, 36];

export const CardCaptureScene: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  const leftEntrance = spring({
    frame,
    fps,
    config: { damping: 200 },
  });

  const rightEntrance = spring({
    frame: frame - 10,
    fps,
    config: { damping: 200 },
  });

  return (
    <AbsoluteFill
      style={{
        backgroundColor: theme.bg,
        padding: 80,
        display: 'flex',
        alignItems: 'center',
      }}
    >
      <div style={{ display: 'flex', flex: 1, alignItems: 'center' }}>
        {/* Left column */}
        <div
          style={{
            flex: 1,
            paddingRight: 40,
            transform: `translateX(${-60 * (1 - leftEntrance)}px)`,
            opacity: leftEntrance,
          }}
        >
          {/* Step number */}
          <div
            style={{
              width: 32,
              height: 32,
              borderRadius: '50%',
              backgroundColor: theme.blue,
              color: '#ffffff',
              fontSize: 16,
              fontWeight: 700,
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
              marginBottom: 16,
            }}
          >
            1
          </div>

          {/* Tag */}
          <div
            style={{
              fontSize: 12,
              fontWeight: 600,
              textTransform: 'uppercase' as const,
              letterSpacing: '0.08em',
              padding: '4px 12px',
              borderRadius: 8,
              backgroundColor: theme.blueSoft,
              color: 'rgb(147,183,255)',
              display: 'inline-block',
              marginBottom: 16,
            }}
          >
            INTAKE
          </div>

          {/* Heading */}
          <div
            style={{
              fontSize: 28,
              fontWeight: 700,
              color: theme.text,
              marginBottom: 12,
            }}
          >
            Business Card Capture
          </div>

          {/* Description */}
          <div
            style={{
              fontSize: 16,
              color: theme.muted,
              lineHeight: 1.7,
              marginBottom: 28,
            }}
          >
            Ambassadors photograph business cards on their phone. The AI extracts
            all contact information and creates a lead record in Monday CRM
            automatically.
          </div>

          {/* Bullet points */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {BULLET_POINTS.map((text, i) => {
              const bulletEntrance = spring({
                frame: frame - BULLET_DELAYS[i],
                fps,
                config: { damping: 200 },
              });

              return (
                <div
                  key={text}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 12,
                    opacity: bulletEntrance,
                    transform: `translateX(${-20 * (1 - bulletEntrance)}px)`,
                  }}
                >
                  <div
                    style={{
                      width: 6,
                      height: 6,
                      borderRadius: '50%',
                      backgroundColor: theme.blue,
                      flexShrink: 0,
                    }}
                  />
                  <span
                    style={{
                      fontSize: 15,
                      color: theme.muted,
                    }}
                  >
                    {text}
                  </span>
                </div>
              );
            })}
          </div>
        </div>

        {/* Right column */}
        <div
          style={{
            flex: 1,
            display: 'flex',
            justifyContent: 'center',
            transform: `translateX(${60 * (1 - rightEntrance)}px)`,
            opacity: rightEntrance,
          }}
        >
          <PhoneMockup />
        </div>
      </div>
    </AbsoluteFill>
  );
};
